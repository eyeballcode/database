...
export default class MongoDatabaseConnection extends DatabaseConnection {
  ...
  async dumpToLoki(targetDB, collectionNames) {
    for (let collectionName of collectionNames) {
      ...
      let outputCollection = await targetDB.getCollection(collectionName)

      let documents = await collection.findDocuments({}).toArray()
      documents = documents.map(obj => JSON.parse(JSON.stringify(obj)))
      await outputCollection.createDocuments(documents)
    }
  }
}
