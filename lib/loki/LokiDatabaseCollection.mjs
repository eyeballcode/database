...
export default class LokiDatabaseCollection extends DatabaseCollection {
    ...
    for (let key of Object.keys(query)) {
      if (query[key].$nearSphere) {
        ...
        for (let i = 0; i < query[key].length; i++) {
          query[key][i] = this.transformQuery(query[key][i])
        }
      } else {
        let match = query[key]
        query[key] = {
  ...
  findDocuments(query) {
    return {
      toArray: () => this.collection.find(this.transformQuery(clone(query)))
    }
  }

  findDocument(query) {
    return this.collection.findOne(this.transformQuery(clone(query)))
  }
