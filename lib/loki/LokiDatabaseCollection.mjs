...
import LokiCursor from './LokiCursor.mjs'
...
export default class LokiDatabaseCollection extends DatabaseCollection {
  ...
  static _fieldMatches(target, query) {
    ...
    for (let op of Object.keys(query)) {
      if (op === '$in') {
        if (target instanceof Array && !target.some(v => query[op].includes(v))) return false
        else if (!(target instanceof Array) && !query[op].includes(target)) return false
  ...
  findDocuments(query, projection) {
    return new LokiCursor(this.collection.find(this.transformQuery(query), projection).map(d => this.project(d, projection)))
  }
  ...
  distinct(field, query) {
    ...
    return Array.from(new Set(matching.flatMap(doc => {
